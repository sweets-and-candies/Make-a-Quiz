...
import {Category, Difficulty, ApiQuestion, Question, Results, QuestionWithAnswer} from './data.models';
import { parseCategories } from './util/quiz-util';
...
export class QuizService {
  ...
  getAllCategories(): Observable<Category[]> {
    return this.http.get<{ trivia_categories: Category[] }>(this.API_URL + "api_category.php").pipe(
      map(res => parseCategories(res.trivia_categories))
    );
  }

  createQuiz(categoryId: string, difficulty: Difficulty, totalQuestions = 5): Observable<Question[]> {
    return this.http.get<{ results: ApiQuestion[] }>(
        `${this.API_URL}/api.php?amount=${totalQuestions + 1}&category=${categoryId}&difficulty=${difficulty.toLowerCase()}&type=multiple`)
      .pipe(
        map(res => {
